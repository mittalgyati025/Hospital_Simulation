...
20% true alarm missed - TrueAlarmsIgnored
90% false alarm ignored understanding it is false - FalseAlarmsIgnored
10% false alarm attended by nurses - FalseAlarmsServiced
*/

public class RunSim5 {
...
//					meclass.departureEvent(custq, ecustq, NextEvent, eventq, custFile, loWriteToCSV, servq);
	...
	System.out.println("Number of True Alarms Missed due to fatigue "+ meclass.TrueAlarmMissed);
	...
	System.out.println("Number of True Alarms Missed  "+ meclass.TrueAlarmsIgnored);
	System.out.println("Number of False Alarms Ignored  "+ meclass.FalseAlarmsIgnored);
	...
	if(pq.size() == 0 && noOfWorkingNurse < noOfNurse-1)
		{
			...
			if(pat[NextEvent.PatientID].alarmType == 0) //True Alarm
			{
				...
				else
				{
					...
					TrueAlarmsIgnored++;
					Event EventObj = new Event("departure", pat[NextEvent.PatientID].arrivalTime);
					EventObj.PatientID = NextEvent.PatientID;
					eq.addEvent(EventObj);
				}
			}
	...
	if(pq.size() > 0) {
		...
		if(pat[pId].alarmType == 0) //True Alarm
		{
			...
			else
			{
				...
				TrueAlarmsIgnored++;
				Event EventObj = new Event("departure", pat[pId].arrivalTime);
				EventObj.PatientID = NextEvent.PatientID;
				eq.addEvent(EventObj);
			}	
		}
