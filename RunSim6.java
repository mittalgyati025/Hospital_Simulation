...
Technician - no fatigue added
*/

public class RunSim6 {
...
//Time taken for nurses to process the alarms
double TrueAlarmProsTime = 0.0165;
double FalseAlarmProsTime = 0.00825;

//Time taken for technicians to process the alarms
double TechTrueAlarmProsTime = 0.0165/2;
double TechFalseAlarmProsTime = 0.00825;
...
//					meclass.departureEvent(custq, ecustq, NextEvent, eventq, custFile, loWriteToCSV, servq);
	...
	System.out.println("Number of False Alarms Missed "+ meclass.DodgedFalseAlarm);
	...
	System.out.println("Number of True Alarms Missed (Result of using technicians)  "+ meclass.TrueAlarmsIgnored);
	System.out.println("Number of False Alarms Dodged (Result of using technicians) "+ meclass.FalseAlarmsIgnored);
...
private Event generateArrivalEvent(EventQ eq,Patient pat[])
{
	...
	//	System.out.println("Time arrival " +TimeArrival );
		...
		if(Math.random() <= 0.06) {
				...
				if(Math.random() <= 0.80) {
					pat[n].ProcessDecision = "Processing";
				}
				else {
					pat[n].ProcessDecision = "NotProcessing";
				}
		} 
		else {
	...
	if(pq.size() == 0 && noOfWorkingNurse < noOfNurse-1)
		{
			...
			if(pat[NextEvent.PatientID].alarmType == 0) //True Alarm
			{
				...
				if(pat[NextEvent.PatientID].ProcessDecision == "Processing")
				{
					...
					TrueAlarmsIgnored++;
					Event EventObj = new Event("departure", pat[NextEvent.PatientID].arrivalTime + TechTrueAlarmProsTime);
			...
			else if(pat[NextEvent.PatientID].alarmType == 1) //False Alarm
			{
				...
				{
						FalseAlarmsIgnored++;
						Event EventObj = new Event("departure", pat[NextEvent.PatientID].arrivalTime +  TechFalseAlarmProsTime);
	...
	if(pq.size() > 0) {
		...
		if(pat[pId].alarmType == 0) //True Alarm
		{
			...
			if(pat[pId].ProcessDecision == "Processing")
			{
				...
				TrueAlarmsIgnored++;
				Event EventObj = new Event("departure", pat[pId].arrivalTime+  TechTrueAlarmProsTime);
				TechTrueAlarmWorkload = TechTrueAlarmWorkload + TechTrueAlarmProsTime;
				EventObj.PatientID = NextEvent.PatientID;
				eq.addEvent(EventObj);
			}	
